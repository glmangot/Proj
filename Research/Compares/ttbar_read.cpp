...
int main(int argc, char* argv[]) {
    ...
    for (int dat = 0; dat < treeNames.size(); dat++) {
    TTree *mu1 = (TTree*)file1->Get(std::to_string(treeNames[treeSeed]).c_str());
    TTree *mu2 = (TTree*)file2->Get(std::to_string(treeNames[treeSeed]).c_str());
    TTree *mu3 = (TTree*)file3->Get(std::to_string(treeNames[treeSeed]).c_str());
    TTree *mu4 = (TTree*)file4->Get(std::to_string(treeNames[treeSeed]).c_str());
