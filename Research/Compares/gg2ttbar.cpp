...
// pT < 3
// all muons id, p4, Iso
// all muons individual iso
// how many > 4 leptons? 
// only save 4 highest pT
...
int main(int argc, char* argv[]) {
    ...
    // pythia.readString("511:onIfAny = 14 -13 -211");
    // pythia.readString("-511:onIfAny = -14 13 211");    
    ...
    // Create fat data matrix
    MRoot  _root;
    // All events are produced here and saved locally. 
    for (int iev = 0; iev < nEvents; iev++) {
        ...
        for (int i = 0; i < ev.size(); i++) {
            ...
            if (ev[i].isFinal()) {
                ...
                // While some of these values are redundant, taking them from pythia
                // saves a lot of calculation time during analyses. With the interpretation
                // that memory is essentially unlimited and can be used to save time
    ...
    for (int iruns = 1; iruns <= 9; iruns++) {
        ...
        TH1F *h_pT_4mu = new TH1F(("h_pT_4mu" + to_string(iruns)).c_str(), ("pT_#mu for #mu in 4#mu events with #eta < |" + to_string(eta_cut) + "|; pT; num").c_str(), 1.3e2, -1, 150);
        TH1F *h_counter= new TH1F(("h_counter" + to_string(iruns)).c_str(), ("Counter for 4#mu events with #eta < |" + to_string(eta_cut) + "|; pT; num").c_str(), 1e2, -1, 10);
        ...
        for (int iev = 0; iev < nEvents; iev++) {
            ...
            for (int i = 0; i < curr_ev.size(); i++) {
                if (abs(get<0>(curr_ev[i])) == 13 && abs(get<1>(curr_ev[i])) < eta_cut) {
                    vector<double> charged, neutral, gammas;
                    for (int j = 0; j < curr_ev.size(); j++) {
                        if (i == j && j != curr_ev.size() - 1) j++;
                        else if (abs(get<1>(curr_ev[j])) < eta_cut && abs(get<1>(curr_ev[i])) < eta_cut && abs(get<0>(curr_ev[j])) != 13) {
                    ...
                    h_iso->Fill(iso);
                    // organize muons then do iso
                    // for muons (only IRL) but we'll pretend it works
                    if (iso < 0.35) {
                        if (get<0>(curr_ev[i]) == 13)
            ...
            if (mu_pos.size() + mu_neg.size() >= 4) {
                for (int k = 0; k < mu_pos.size(); k++) 
                    h_pT_4mu->Fill(get<4>(mu_pos[k]));
                for (int k = 0; k < mu_neg.size(); k++)
                    h_pT_4mu->Fill(get<4>(mu_neg[k]));
                h_counter->Fill(mu_pos.size() + mu_neg.size());    
